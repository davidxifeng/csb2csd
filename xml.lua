...
function Doc:set_attribs (data, template)
  data = data or {}
  if template then
    for _, k in ipairs(template) do self.attr[k] = data[k] end
  else
    for k, v in pairs(data) do self.attr[k] = v end
  end
  return self
end
...
-- @param items either text or a table where the hash part is the attributes and the list part is the children.
-- @within module
function _M.doc(tag, attr, items)
    local s = _M.new(tag, attr)
...
-- @param tag the tag name
-- @param[opt] create add a child when not found such a child
-- @within Doc
function Doc:child_with_name(tag, create)
    for _, child in ipairs(self) do
        if child.tag == tag then return child end
    end
    if create then
        local child = _M.new(tag)
